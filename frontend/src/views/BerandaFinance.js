...
function BerandaFinance() {
  ...
  useEffect(() => {
    const fetchYears = async () => {
      try {
        const response = await axios.get("http://locahost:5000/filter-piutang", {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
    ...
    const fetchUserData = async () => {
      ...
      try {
        const response = await axios.get(
          `http://locahost:5000/user-details/${username}`,
          {
    ...
    const fetchSummaryData = async () => {
      try {
        const [responseTotalPinjaman, responseTotalPeminjam, responseTotalDibayar, responsePlafond] = await Promise.all([
          axios.get("http://locahost:5000/total-pinjaman-keseluruhan", {
            headers: { Authorization: `Bearer ${token}` },
          }),
          axios.get("http://locahost:5000/total-peminjam", {
            headers: { Authorization: `Bearer ${token}` },
          }),
          axios.get("http://locahost:5000/total-dibayar", {
            headers: { Authorization: `Bearer ${token}` },
          }),
          axios.get("http://locahost:5000/latest-plafond-saat-ini", {
            headers: { Authorization: `Bearer ${token}` },
          }),
        ]);
  ...
  const getPinjamanData = async () =>{
    try {
      // setLoading(true);
      const response = await axios.get("http://locahost:5000/pinjaman-data", {
        headers: {
            Authorization: `Bearer ${token}`,
        },
      });
  ...
  const handleTerimaClick = async (pinjaman) => {
    ...
    try {
      ...
      const response = await axios.put(`http://locahost:5000/pinjaman/${pinjaman.id_pinjaman}`, {
        status_pengajuan: pinjaman.status_pengajuan,
        status_transfer: "Selesai", 
        id_asesor: userData.id_karyawan, 
      }, {
  ...
  const dataPinjaman = async (selectedYear) => {
    try {
      const response = await axios.get("http://locahost:5000/data-pinjaman", {
        headers: {
          Authorization: `Bearer ${token}`,
        },
        params: {
  ...
  const dataPerDivisi = async (selectedDepartemen, selectedMonth = "", selectedYear = "") => {
    try {
      ...
      const response = await axios.get("http://locahost:5000/data-divisi", {
        headers: {
          Authorization: `Bearer ${token}`,
        },
        params: {
  ...
  const dataPeminjamPerDivisi = async (selectedDepartemenPeminjam, selectedMonthPeminjam = "", selectedYear = "") => {
    try {
        ...
        const response = await axios.get("http://locahost:5000/data-peminjam-per-divisi", {
            headers: {
                Authorization: `Bearer ${token}`,
            },
            params: {
