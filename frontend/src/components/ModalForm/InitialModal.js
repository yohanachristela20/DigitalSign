...
const InitialModal = ({showInitialModal, setShowInitialModal, onSuccess, selectedIdItem, selectedSigner, show, editable}) => {
            ...
            try {
                ...
                for (const selectedSigner of signerArray) {
                    for (const itemID of itemArray) {
                        const response = await axios.get(`http://10.70.10.20:5000/axis-field/${id_dokumen}/${selectedSigner}/${itemID}`);
    ...
    const today = new Date();
        ...
        try {
            ...
            const response = await axios.patch(`http://10.70.10.20:5000/initialsign/${id_dokumen}/${id_item}/${id_signers}`, {
                sign_base64,
                status: "Completed", 
                tgl_tt: today,
            }, {
