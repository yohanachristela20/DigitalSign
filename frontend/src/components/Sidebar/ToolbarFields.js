...
//   const [activeTool, setActiveTool] = useState(localStorage.getItem('currentTool') || null);
...
//     const current = localStorage.getItem('currentTool') || '';
//     if (current === toolName) {
//       // turn off
//       setActiveTool(null);
//       localStorage.setItem('currentTool', '');
//       localStorage.setItem('signatureClicked', 'false');
//       localStorage.setItem('initialClicked', 'false');
//       localStorage.setItem('dateFieldClicked', 'false');
//     } else {
//       // turn on requested tool
//       setActiveTool(toolName);
//       localStorage.setItem('currentTool', toolName);
//       localStorage.setItem('signatureClicked', toolName === 'Signpad' ? 'true' : 'false');
//       localStorage.setItem('initialClicked', toolName === 'Initialpad' ? 'true' : 'false');
//       localStorage.setItem('dateFieldClicked', toolName === 'Date' ? 'true' : 'false');
//     }
...
//     if (!id_karyawan) {
//       alert("Please choose signer first.");
//       return;
//     }
...
//     updateClickedFields(toolName);

//     window.dispatchEvent(new Event("localStorageUpdated"));
//   };

//   const handleAddSignature = () => handleAddField("Signpad");
//   const handleAddInitials = () => handleAddField("Initialpad");
//   const handleAddDate = () => handleAddField("Date");
...
//         alert('Previous signer should be sign the document first.');
//         return;
//       }
//     }
...
//         },
//       });
//       const newCategory = response.data.map((item) => ({ value: item.id_kategoridok, label: item.kategori }));
...
//         ? data.map((item, index) => ({ value: item.id_karyawan, label: item.Penandatangan?.nama || `Signer ${index + 1}`, color: identitycolor[index % identitycolor.length] }))
//         : [];
...
//     localStorage.setItem('signersWithSignature', JSON.stringify(signersWithSignature || []));

//     localStorage.setItem('currentTool', activeTool || '');

//     window.dispatchEvent(new Event('localStorageUpdated'));
//   }, [signersDoc, signersCount, selectedOption, signersWithSignature, activeTool]);
...
//     localStorage.setItem('currentSigner', event.target.value);
//     window.dispatchEvent(new Event('localStorageUpdated'));
//   };
...
//     {
//       name: 'My Documents',
//       children: Array.isArray(signersDoc) ? signersDoc.map((item) => ({ name: item.id_signers })) : [],
//     },
//   ];

//   const activeRoute = (routeName) => location.pathname.includes(routeName);

//   const filteredRoutes = routes.filter((route) => !['/screening-karyawan', '/login', '/surat-pernyataan', '/document', '/preview-doc'].includes(route.path));

//   const renderMenuItems = (routesList, parentKey = '') => {
...
//           {hasChildren && isOpen && <ul className="submenu-items">{renderMenuItems(route.children, key + '-')}</ul>}
//         </li>
//       );
//     });
//   };
...
//             <input type="checkbox" checked={isSidebarCollapsed} onChange={(e) => setIsSidebarCollapsed(e.target.checked)} />
//             <img src={require('assets/img/logo4.png')} alt="sidebar-logo" style={{ width: '30px' }} className="img-items" />
//           </label>
...
//                 <div className="identity-color" style={{ backgroundColor: selectedOption?.color || '' }} />
...
//         </a>
//         <ul className="items">
//           <Button type="button" className={`btn mb-3 bg-transparent mt-2 ${activeTool === 'Signpad' ? 'active-tool' : ''}`} style={{ width: '230px', border: '1px solid #eed4dd' }} onClick={handleAddSignature}>
//             <i className="fa fa-signature" style={{ marginRight: '8px' }} />
//             Signature
//           </Button>
//           <Button type="button" className={`btn mb-3 bg-transparent mt-2 ${activeTool === 'Initialpad' ? 'active-tool' : ''}`} style={{ width: '230px', border: '1px solid #eed4dd' }} onClick={handleAddInitials}>
//             <i className="fa fa-font" style={{ marginRight: '8px' }} />
//             Initial
//           </Button>
//           <Button type="button" className={`btn mb-3 bg-transparent mt-2 ${activeTool === 'Date' ? 'active-tool' : ''}`} style={{ width: '230px', border: '1px solid #eed4dd' }} onClick={handleAddDate}>
