...
// const PDFCanvas = ({pdfUrl, currentPage, onPageRender}) => {
//     console.log("currentPage from receive:", currentPage);
...
//                     await page.render(renderContext).promise;
//                     renderedCanvases.push({pageCurrent, pageNumber: pageNum, imgSrc: canvas.toDataURL()});
//                 }
//                 setCanvases(renderedCanvases);
//             } catch (error) {
//                 console.error('Error loading or rendering PDF: ', error);
...
//       <div ref={canvasContainerRef}>
//         {canvases.length === 0 && <p>Loading PDF...</p>}
//         {canvases.map(({ pageNumber, imgSrc }) => {
//             const divId = `${pageNumber}`;
//             console.log("Rendering div with id:", divId, "pageCurrent:", pageCurrent, "pageNumber:", pageNumber);
...
//             {/* {pageCurrent !== null && Number(pageCurrent) === Number(pageNumber) && (
//               <> */}
//             {signatures
//             .filter((sig) => Number(sig.page) === Number(pageNumber)) 
//             .map((sig) => (
//               <ResizableDragable
//                 key={sig.id}
//                 id={sig.id}
